...
class VllmConfig:
    ...
    """Maximum number of distinct adapters simultaneously active on GPU.
    Default is high because adapters are tiny (~16K params/layer)."""
    max_cpu_adapters: int = 1024
    """Maximum number of adapters cached on CPU. Adapters are lazily
    loaded to GPU when needed and LRU-evicted when at ``max_adapters``."""
    ...
    def compute_hash(self) -> str:
        ...
        if self.adapter_config:
            # adapter tensor shapes are baked into compiled graph inputs
            ...
            def _shape_of(v):
                ...
                # serialized {"__adapter_t": True, "shape": [...], ...} dicts
                # (post-spec_to_adapter_config).
                if isinstance(v, dict) and v.get("__adapter_t"):
            ...
            vllm_factors.append(hashlib.md5(
                json.dumps(adapter_factors, sort_keys=True, default=str).encode(),
